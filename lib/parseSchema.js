...
const objectMethods = require('./methods/object');
...
function astObjectToAjvSchema(root) {
	...
	var flush = function () {
		...
		methods.set(obj, [t.objectExpression(props)], {clone: false});
		props = [];
	...
	root.properties.forEach(function (prop) {
		if (t.isSpreadElement(prop)) {
			...
			methods[isPure(src) ? 'set' : 'merge'](obj, [src], {clone: false});
			...
			if (methodPropName.test(name)) {
				...
				if (objectMethods.hasOwnProperty(method)) {
					objectMethods[method](obj, [prop.value], {clone: false});
					restoreProps();
					return;
				}
			}
...
function astArrayToAjvSchema(root) {
	...
	case 1:
		...
		if (t.isSpreadElement(element)) {
			...
			arr.properties.push(
				t.objectProperty(
					t.stringLiteral('contains'),
					astToAjvSchema(element.argument)
				)
			);
		}
		else {
			items.value = astToAjvSchema(elements[0]);
		}
		break;
	default:
		...
		items.value = t.arrayExpression(elements.filter(el => !t.isSpreadElement(el)).map(astToAjvSchema));

		if (spreads.length > 0) {
			arr.properties.push(
				t.objectProperty(
					t.stringLiteral('contains'),
					astToAjvSchema(spreads[0].argument)
				)
			);
		}
