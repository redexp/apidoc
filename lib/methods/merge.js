...
module.exports = function merge(target, schemas, {methodName = 'merge', createNew = true} = {}) {
	method(methodName);
	atLeastOne(schemas);
	...
	if (createNew) {
		target = clone(target);
		target.properties = clone(target.properties);

		target.properties.forEach(function (prop, i) {
			if (prop === required || prop === properties) return;

			target.properties[i] = cloneDeep(prop);
		});
	}
