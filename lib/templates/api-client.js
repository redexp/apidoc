...
Api.request = function ajax({url, query, body, context}) {
...
function createRequest(context, e) {
	...
	return async function () {
		...
		if (e.params && e.params.names.length > 0) {
			if (typeof args[0] === 'object') {
				...
				e.params.names.forEach(function (name, i) {
					params[name] = args[i];
				});

				args = args.slice(e.params.names.length);
			}
		}
